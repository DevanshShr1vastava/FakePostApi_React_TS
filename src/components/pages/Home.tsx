...
const Home = () => {
  ...
  const filteredData = selectedCategory ? product?.filter((prod:IProductData)=> prod.category === selectedCategory) : product;
  ...
  <DropdownMenuTrigger asChild>
    <Button className="mr-4" variant="destructive">Category Filter</Button>
  </DropdownMenuTrigger>
  <DropdownMenuContent className="w-56 bg-gray-200 dark:bg-gray-800 shadow-md rounded-md">
    <DropdownMenuSeparator />
    <DropdownMenuRadioGroup value={selectedCategory} onValueChange={setSelectedCategory}>
      <DropdownMenuRadioItem value={""} onClick={() => setSelectedCategory(undefined)}>All</DropdownMenuRadioItem>
  ...
  <div className="grid grid-cols-4 gap-6">
    {filteredData?.map((prod: IProductData) => (
      <Card key={prod.id} className="h-full flex flex-col">
        <CardHeader>
          <CardTitle className="text-lg truncate font-semibold">{prod.title}</CardTitle>
        </CardHeader>
            ...
            <AlertDialog>
              ...
              <AlertDialogContent>
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction onClick={() => handleDelete(prod.id)}>Continue</AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
            </AlertDialog>
          </div>
